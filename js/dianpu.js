$(function(){
    ...
    $.ajax({
        type:"post",
        url:"php/service_details.php",
        data:{agentId:agentId},
        success:function(d){
            console.log(d);
            $(".dianpu_head h2").html('客服'+d.agentName+'的店铺');
            $('.dianpu_service img').attr('src',d.agentPic);
            $('.dianpu_service h3').text(d.agentName);
            $('.dianpu_service p').append('<span>'+d.areaName+'</span><span>'+d.subAreaName+'</span>');
            $('.dianpu_service strong').append(d.agentPhone);
        }
    });
        ...
        $.ajax({
            ...
            success:function(d){
                ...
                for(var i=0;i<data.length;i++){
                    htmlStr+=`<li>
                                <a href="zf_details.html?zfId=${data[i].zfId}"><img src="${data[i].picList[i].zfPic}" alt=""/></a>
                                ...
                                <p>${data[i].decoration}</p>
                                <span><strong>${data[i].price}</strong>元/晚</span>
                                </li>`;
                }
                $(".fangyuan_list").html(htmlStr);
                $('.dianpu_head .zf').html('控房房源：'+d.totalRecord+'套');
