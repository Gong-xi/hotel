$(function() {
    ...
    $.ajax({
        ...
        success:function(d){
            ...
            $(".zf_details_head p").html('<span>酒店编号：'+d.zfId+'</span><span> 更新时间：'+dateFormat(t,'-')+'</span>');
            $('.details_info>h3>span').text(d.price);
            var liStr=`<li class="left"><span>酒店：</span>${d.title}</li>
            <li class="left"><span>房型：</span>${d.houseType}</li>
                        <li class="left"><span>面积：</span>${d.size}㎡</li>
                        <li class="left"><span>押金：</span>${d.pay}</li>
                        `;
