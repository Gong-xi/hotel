            ...
            success:function(d){
                ...
                for(var i=0;i<data.length;i++){
                    htmlStr+=`<li class="clearfloat">
                            <a href="zf_details.html?zfId=${data[i].zfId}" target="_blank"><img src="${data[i].picList[i].zfPic}" alt=""/></a>
                            <div class="information">
                                <h2><a href="zf_details.html?zfId=${data[i].zfId}" target="_blank">${data[i].title}</a></h2>
                                <p><span>${data[i].houseType}</span><span>${data[i].leaseWay}</span><span>${data[i].size}㎡</span><span>装修：${data[i].decoration}</span></p>
                            </div>
